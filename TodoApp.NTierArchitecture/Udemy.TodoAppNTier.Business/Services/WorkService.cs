...
using Udemy.TodoAppNTier.Business.Extension;
...
namespace Udemy.TodoAppNTier.Business.Services
{
    public class WorkService : IWorkService
    {
        ...
        private readonly IValidator<WorkCreateDto> _createValidatorDto;
        private readonly IValidator<WorkUpdateDto> _updateValidatorDto;
        public WorkService(IUow uow, IMapper mapper, IValidator<WorkCreateDto> createValidatorDto, IValidator<WorkUpdateDto> updateValidatorDto)
        {
            _uow = uow;
            _mapper = mapper;
        ...
        {
            //validator.Validate(dto); ile validation resultl döndürüyoruz.
            var validationResult = _createValidatorDto.Validate(dto);
        ...
        {
            //Burada GetByFilter() ile tracking engelledik.
        ...
        {
            var result = _updateValidatorDto.Validate(dto);
            if (result.IsValid)
            {
                ...
                return new Response(ResponseType.NotFound, $"{dto.Id} ye ait data bulunamadı");
            }
            else
            {
